...
# USE_HIVE is false by default so the pipeline runs without Hive metastore
# initialization. Set $env:USE_HIVE="true" before calling the script to enable it.
$useHive = $env:USE_HIVE
if (-not $useHive) { $useHive = "false" }
...
if ($StressTest) {
    ...
    # Shuffle partitions: 2–3× cores baseline (driver also uses SPARK_SHUFFLE_MULTIPLIER=3 by default).
