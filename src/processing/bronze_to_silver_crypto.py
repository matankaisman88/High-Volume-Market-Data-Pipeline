...
Changes for high-volume / stress-test mode:
  ...
  • Silver Delta table is partitioned by partition_date to balance partition
    pruning with file counts and metadata overhead.
...
def _transform(df: DataFrame) -> DataFrame:
    typed_df = df.select(
        *[
            col(c)
            for c in df.columns
            if c not in ("last_updated", "current_price", "market_cap")
        ],
        to_timestamp(col("last_updated")).alias("last_updated"),
        col("current_price").cast(DecimalType(18, 8)).alias("current_price"),
        col("market_cap").cast(DecimalType(20, 2)).alias("market_cap"),
    )
...
def _upsert_to_silver(spark: SparkSession, df: DataFrame, logger: logging.Logger) -> None:
    """
    Write or merge silver data, partitioned by partition_date.

    New table → full overwrite with schema evolution.
    Existing  → MERGE on (id, last_updated); update matched rows, insert new ones.
    Partitioning by partition_date enables efficient time-range pruning while
    avoiding excessive small files from symbol-level partitioning.
    """
