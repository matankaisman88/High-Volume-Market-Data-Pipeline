...
from src.config.paths import SILVER_PATH, GOLD_PATH, FINAL_REPORT_CSV_PATH
...
def _compute_market_stats(df: DataFrame) -> DataFrame:
    ...
    For EACH partition_date:
      1. Repartition by partition_date so Window and groupBy are partition-local (reduces shuffle).
      2. Rank coins by total_volume (descending).
      3. Keep the top-10 by volume.
      4. Aggregate: total_market_cap = SUM(market_cap),
                    avg_price        = AVG(current_price).

    Returns one row per partition_date — a 30-day daily trend for stress-test runs.
    Uses only Spark native functions; no UDFs.
    """
    if df.rdd.isEmpty():
        return df.limit(0)

    # Align partitions with partition_date so Window + groupBy are partition-local (avoids massive shuffle).
    # If adding small metadata joins later, use .hint("broadcast") on the small side.
    df = df.repartition("partition_date")
...
def _write_gold(spark: SparkSession, df: DataFrame, logger: logging.Logger) -> None:
    """Write gold stats, partitioned by partition_date. Repartition by partition_date
    before write to avoid small-file fragmentation (one file per date)."""
    # Coalesce to one partition per partition_date for efficient Delta writes and data skipping
    df_for_write = df.repartition("partition_date")
    (
        df_for_write.write
        .format("delta")
        .mode("overwrite")
...
def _export_final_csv(df: DataFrame, logger: logging.Logger) -> None:
    ...
    pdf = df.toPandas()
    out_path = FINAL_REPORT_CSV_PATH
...
def run_silver_to_gold(
    ...
    silver_path supplied → read from Delta path (preferred after Silver OPTIMIZE).
    Otherwise uses default SILVER_PATH.
    ...
    try:
        if silver_df is not None:
            ...
            path = silver_path or SILVER_PATH
            source_df = spark.read.format("delta").load(path)
            logger.info("Read silver from Delta: %s", path)

        stats_df = _compute_market_stats(source_df)
        logger.info("Market stats computed.")

        # Single partition for ~30 rows to avoid driver managing many file commits
        stats_df = stats_df.coalesce(1)
